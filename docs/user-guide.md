...
**Teacher Assistant** is a free, offline-first application designed to help teachers manage their classrooms efficiently. It works entirely on your computerâ€”no internet required after installation.

### What You Can Do:
- âœ… Track daily attendance
- âœ… Manage student rosters
- âœ… Generate attendance reports
- âœ… Share classes with other teachers
- âœ… Create seating charts
...
- **Username:** `admin`
- **Password:** value of `DEFAULT_ADMIN_PASSWORD` from your `.env` (generated by setup script or set manually)
...
2. Click the **âš™ï¸ gear icon** next to "Current Class"
...
> ðŸ’¡ **Tip:** You can rename or delete classes from the edit mode (gear icon).
...
2. Click **Excel Tools** â†’ **Download Template**
3. Open the downloaded template in Excel
4. Fill in your students (one per row)
5. Save the file
6. Click **Excel Tools** â†’ **Import from Excel**
7. Select your file

> ðŸ’¡ **Tip:** Roll numbers must be unique. The app will update existing students if roll numbers match.

### Flagging Students
- Click the **ðŸš© flag icon** next to a student to mark them
...
### Bulk Add Teachers
1. In Admin Dashboard â†’ Teachers tab
...
> ðŸ’¡ **Tip:** Back up weekly or before major changes.
...
**Solution:**
1. Default login is `admin` and the password is your configured `DEFAULT_ADMIN_PASSWORD`
