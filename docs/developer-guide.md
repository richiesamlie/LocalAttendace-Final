...
```bash
# Development
bun run dev              # Dev server (localhost)
bun run dev:network      # Dev server (LAN access)

# Production
bun run build            # Build frontend
bun run start            # Production server
bun run start:network    # Production server (LAN access)

# TypeScript
bun run lint             # tsc --noEmit

# Database
bun run db:backup        # Backup to backups/
bun run db:restore       # Restore from backup
bun run db:seed          # Seed with test data
bun run db:fresh         # Fresh start (backup + restore)
...
- `chore(module): description` — Build, deps, CI/CD
