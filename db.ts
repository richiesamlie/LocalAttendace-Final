...
const initSchema = () => {
  ...
  if (!hasTeacherId) {
    ...
    const existingTeachers = preparedStatements.countTeachers.get() as { count: number };
    let defaultTeacherId = 'teacher_default';
    if (existingTeachers.count === 0) {
      const hash = bcrypt.hashSync('teacher123', 10);
      preparedStatements.insertTeacher.run(
        defaultTeacherId, 'admin', hash, 'Administrator'
      );
    } else {
      const teacher = preparedStatements.getFirstTeacher.get() as { id: string };
  ...
  const teacherCount = preparedStatements.countTeachers.get() as { count: number };
