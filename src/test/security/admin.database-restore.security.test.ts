...
describe('Admin database restore endpoint security', () => {
  ...
  async function mintAdminCookie(): Promise<string> {
    ...
    await sessionService.insert(sessionId, teacher.id, 'vitest', '127.0.0.1', expiresAt);

    const token = jwt.sign({ teacherId: teacher.id, username: teacher.username, sessionId }, JWT_SECRET, { expiresIn: '1h' });
